...
int main(){
	...
	auto start_time = high_resolution_clock::now(); //to calculate time
	//loading data into vectors 
	CSVReader raw_data("breast-cancer.data");
...
//everything before this was dataloading and datasplitting
	...
	while((tot_gamma < 1) && (reduct.size() <= feat_count) && (pos_count < obj_count)){
		...
		if(bsp_pid() == 0) {
			...
			cout<<"granule is "<<granule<<endl;
			cout<<"current reduct is \t"<<endl;
			for(int i = 0; i < reduct.size(); i++) cout<<reduct[i]<<"\t";
			cout<<endl;
			//cout<<"size is "<<reduct.size();
		}
		vector<double> loc_gammas(nsub_feat, 0.0); //a vector to store all the local gamma values; initialize to the number of features
	...
	*/
	...
	auto stop_time = high_resolution_clock::now(); //duration until here
